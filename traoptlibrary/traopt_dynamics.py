...
class SO3Dynamics(BaseDynamics):
    ...
    """Error-State SO(3) Dynamics Model"""
    ...
    def __init__(self, J, dt, integration_method="euler",
        ...
        Args:
            J: diag Inertia matrix, moment of inertia in the body frame.
            dt: Sampling time.
...
class Pendulum3dDyanmics(BaseDynamics):

    """A dynamics model for 3d pendulum actuated by the pivot point"""

    def __init__(self, J, m, length, dt,  integration_method="euler",
                    state_size=(3,3), action_size=3, 
                    hessians=False, debug = None, 
                    **kwargs):
        """Constructs an Dynamics model for Pendulum3D based on SO3.

        Args:
            J: diag Inertia matrix, I_b, 
            m: mass of the system,
            length: length of the pendulum system
            dt: Sampling time.
            integration_method: integration method for dynamics,
                "euler": euler method,
                "rk4": Runga Kutta 4 method.
        ...
        super(Pendulum3dDyanmics, self).__init__()
    ...
    def fd_euler( self, x, u, i ):
        ...
        g_term = SO3Tangent(self.m * self.g * rho).smallAdj() @ q.inverse().act(down_vec)
        g_term = g_term.reshape(self.vel_state_size, 1)

        M = SO3Tangent(self.m * rho).smallAdj() @ q.inverse().act(u)
        M = M.reshape(self.vel_state_size, 1)
        ...
        xi_next = SO3Tangent(
            vec_xi 
            + self.Jinv @ ( xi.smallAdj().T @ self.J @ vec_xi + g_term + M ) * self.dt  
            # + self.Jinv @ ( xi.smallAdj().T @ self.J @ vec_xi + g_term ) * self.dt  
            # + self.Jinv @ ( xi.smallAdj().T @ self.J @ vec_xi + g_term + u ) * self.dt  
        )
        return [q_next, xi_next]
    ...
    def f_x(self, x, u, i):
        ...
        J_qTe3_qT = np.empty((self.vel_state_size, self.pos_state_size))
        J_q_inv = np.empty((self.pos_state_size, self.pos_state_size))
        _ = q.inverse( J_q_inv ).act( down_vec, J_qTe3_qT )
        L1 = skew( self.m * self.g * rho ) @ J_qTe3_qT @ J_q_inv

        J_qTu_qT = np.empty((self.vel_state_size, self.pos_state_size))
        J_q_inv = np.empty((self.pos_state_size, self.pos_state_size))
        _ = q.inverse( J_q_inv ).act( u, J_qTu_qT )
        L2 = skew( self.m * rho ) @ J_qTu_qT @ J_q_inv
        ...
        return np.block([
                    [J_q_q,         J_q_xi],
                    [L*self.dt,     np.identity(3) + H*self.dt],
                ])
    ...
    def f_u(self, x, u, i):
        ...
        J_RTu_u = np.empty((self.vel_state_size, self.pos_state_size))
        _ =  q.inverse().act(down_vec, None, J_RTu_u)

        # bt = self.Jinv @ SO3Tangent( self.m * rho ).smallAdj() @ q.inverse().rotation()
        bt = self.Jinv @ SO3Tangent( self.m * rho ).smallAdj() @ J_RTu_u
        
        # bt = self.Jinv

        Bt = np.vstack(
            (np.zeros((self._pos_state_size, self.action_size)), bt )
        )
...
class RigidBodyDynamics(BaseDynamics):

    """SE(3) Dynamics with Gravity"""
    ...
    def fd_euler( self, x, u, i ):
        ...
        q_next = manifSE32SE3( q_mnf.rplus(xi_mnf * self.dt) ) 
        # q_next = q @ scipy.linalg.expm( se3_hat(xi) * self.dt )
        xi_next = xi + self.Jinv @ ( coadjoint( xi ) @ self.J @ xi + g_acc + u ) * self.dt  
