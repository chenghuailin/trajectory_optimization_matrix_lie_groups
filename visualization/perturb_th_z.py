...
def main():
    ...
    th_z_max = 210.
    th_z_step = 0.5
    th_z_range = np.arange(th_z_min, th_z_max, th_z_step)  
    
    # th_z_range = np.arange(150., 210., 0.5)
    # th_y_range = np.arange(150., 210., 0.5)
    # th_x_range = np.arange(150., 210., 0.5)
    # w_x_range = np.arange(-3, 3, 0.05)
    # w_y_range = np.arange(-2, 2, 0.05)
    # w_z_range = np.arange(-1., 1., 0.05) + 1.   

    # p_x_range = np.arange(-50., 50., 2.)
    # p_y_range = np.arange(-50., 50., 2.)
    # p_z_range = np.arange(-50., 50., 2.)
    # v_x_range = np.arange(-50., 50., 2.)
    # v_y_range = np.arange(-50., 50., 2.)
    # v_z_range = np.arange(-50., 50., 2.)
    ...
    save_path = os.path.join(visualization_dir, 
                             f'results_th_z_{int(th_z_min)}_{int(th_z_max)}.npz')
    ...
    for i, traj in enumerate(results_th_z['trajectories']):
        ...
        pos = np.array([element[0][:3, 3] for element in traj])
