...
def rollout_nominal(dynamics, x0, us):
    N = us.shape[0]
    xs = [None] * (N + 1)
    xs[0] = x0
    for i in range(N):
        xs[i+1] = dynamics.f(xs[i], us[i], i)
    return xs

def rollout_parameter(param_name, param_value, dynamics_params, N, action_size, HESSIANS, debug_dyn):
    try:
        ...
        xs_nom = rollout_nominal(dynamics, x0, us_init)

        print(f"Parameter {param_name}={param_value} optimization completed")
        return param_name, param_value, xs_nom
...
def main():
    ...
    # =====================================================
    print("Starting parallel rollout for multiple parameters")
    ...
    for param_name, param_values in parameter_ranges.items():
        print(f"Starting parallel rollout for parameter: {param_name}")
        results_all = Parallel(n_jobs=num_jobs, verbose=5)(
            delayed(rollout_parameter)(
                param_name, param_value, dynamics_params, 
    ...
    print(f"Rollout for all parameters finished! Time used:{time.time()-start_time}")
    # =====================================================
